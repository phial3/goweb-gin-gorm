...
var (
	Discard = New(log.New(ioutil.Discard, "", log.LstdFlags), internalConfig{})
	Default = New(log.New(os.Stdout, "\r\n", log.LstdFlags), internalConfig{
...
func New(writer logger.Writer, config internalConfig) logger.Interface {
	var (
		infoStr      = "%s\n[info] "
		warnStr      = "%s\n[warn] "
		errStr       = "%s\n[error] "
...
func (c *_logger) Printf(message string, data ...interface{}) {
	if GlobalConfig.Mysql.LogZap {
		GlobalLog.Info(fmt.Sprintf(message, data...))
	} else {
